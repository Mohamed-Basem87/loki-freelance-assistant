...
LABEL org.opencontainers.image.description="Freelance job monitor: Telegram + FreeHub ingestion, keyword classifier, Gemini/Groq review, Excel audit log, Telegram notifications."
...
# Runtime data dirs: Telethon session, Excel audit log, dedup state.
RUN mkdir -p /app/sessions /app/logs /app/database \
    && useradd --create-home --uid 1000 loki \
    && chown -R loki:loki /app

USER loki

# Mount volumes (named volumes recommended) so the session, log and
# state survive container restarts / rebuilds.
VOLUME ["/app/sessions", "/app/logs", "/app/database"]
