...
directly -- it checks external resources (Postgres, Redis) plus
files on disk. It checks three genuinely distinct things and reports
which one failed, rather than folding them into a single "backend is
reachable" signal:

  1. Persistence integrity  -- Postgres (DATABASE_URL) answers a
     trivial `SELECT 1`, Redis (REDIS_URL) answers `PING`, and the
     JSON state file (if present) parses. This is a reachability
     check, not a schema/data-integrity check -- the same shallow
     depth the old SQLite `PRAGMA quick_check` had (it wouldn't have
     caught row corruption either).
  2. Process health         -- the container's process table is
     observable at all (a minimal, cheap sanity check).
  3. Worker/runtime health  -- app.infra.heartbeat.heartbeat_loop, a
...
def expected_critical_workers(enabled_workers=None, job_sources=None):
    ...
    """
    import app.infra.runtime_config as _rc
...
# Short, bounded timeouts: this runs as a Docker HEALTHCHECK subprocess
# on a tight interval, so a slow/unreachable backend must fail fast
# rather than hang the probe.
_CONNECT_TIMEOUT_SECONDS = 2


def check_persistence_integrity(database_url, redis_url, state_path):
    try:
        with psycopg.connect(database_url, connect_timeout=_CONNECT_TIMEOUT_SECONDS) as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT 1")
    except Exception as exc:
        raise RuntimeError(f"persistence integrity: postgres unreachable: {exc}") from exc
    ...
    try:
        ...
        raise RuntimeError(f"persistence integrity: redis unreachable: {exc}") from exc
    finally:
        client.close()
...
def check_worker_runtime_health(
    ...
    #    worker data (the JSON format written by app.infra.heartbeat.WorkerLiveness).
...
def main():
    ...
    if not database_url or not redis_url:
        print(f"{UNHEALTHY}: persistence integrity: DATABASE_URL/REDIS_URL not configured")
        raise SystemExit(1)

    try:
        check_persistence_integrity(database_url, redis_url, state_path)
    except RuntimeError as exc:
        print(f"{UNHEALTHY}: {exc}")
        raise SystemExit(1)
