...
RUNTIME = RuntimePolicy(
    ...
    # app.infra.heartbeat.heartbeat_loop keeps ticking on schedule and the
...
# app.infra.timeouts). HTTP_TIMEOUT_SECONDS is the SDK/HTTP-layer deadline
...
# docstring on why app.infra.config defers its required-credential checks to
# lazy getter functions instead of raising at import time). Requiring
# them here would make importing app.infra.runtime_config itself fail for
# any caller that hasn't set them yet, which is exactly the ordering
# problem app.infra.config's lazy pattern exists to avoid. The composition
